    ...
    <>
      ...
      <ModalBox title={task.title} show={details} close={() => setDetails(false)}>
        <DropDownBox>
          ...
          <Dropdown.Item className="color-red" onClick={deleteTaskPopup}>
            Delete Task
          </Dropdown.Item>
        </DropDownBox>
        ...
        <TaskDesc>
          <SubTaskTitle>
            Subtasks ({completedTasks.length} of {task.subtasks.length})
          </SubTaskTitle>
          {task.subtasks.map((subtask) => (
            <SubTask key={subtask.title} el={subtask} handler={changeSubTaskHandler} />
          ))}
        </TaskDesc>
